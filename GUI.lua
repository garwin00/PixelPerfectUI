...
-- Layout constants
local WIN_W  = 480
local PAD    = 12    -- horizontal padding from window edge
local ROW_H  = 18    -- standard label/value row height
local GAP    = 12    -- space between sections

-- =============================================================================
-- WIDGET HELPERS
-- =============================================================================

--- Create a Frame with a solid backdrop (Shadowlands+ requires "BackdropTemplate")
local function NewBackdrop(parent, name)
    return CreateFrame("Frame", name, parent, "BackdropTemplate")
end

--- Apply backdrop colours to a BackdropTemplate frame.
...
--- One-pixel horizontal divider line.
...
--- Small accent-coloured section heading.
...
--- Left-aligned dim label for an info row.
...
--- Right-aligned value FontString for an info row (returned for later updates).
...
--- Checkbox with a text label to its right.
...
--- Custom horizontal slider (avoids template naming conflicts).
local function MakeSlider(parent)
    ...
    -- Soft enable/disable (Slider doesn't have Enable/Disable like Button)
    f._enabled = true
...
function PPUI:CreateGUI()
    ...
    win:SetSize(WIN_W, 10)   -- height calculated and set at bottom of this function
    ...
    verFS:SetPoint("RIGHT", hdr, "RIGHT", -36, 0)
    ...
    -- ── Y cursor (negative = distance below TOPLEFT of window) ───────────────
    local Y = -44    -- first row starts 44px from top (8px below 36px header)

    -- =========================================================================
    -- SECTION: STATUS
    -- =========================================================================
    SectionTitle(win, "Status", Y);  Y = Y - 16

    -- Status indicator: coloured dot + bold text
    local statusDot = win:CreateTexture(nil, "ARTWORK")
    statusDot:SetSize(10, 10)
    statusDot:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y - 4)
    statusDot:SetColorTexture(T.bad[1], T.bad[2], T.bad[3], 1)

    local statusFS = FS(win, 13, "OUTLINE")
    ...
    -- Info rows
    win._vPhysRes = RowValue(win, Y); RowLabel(win, "Physical Resolution",      Y); Y = Y - ROW_H
    win._vAspect  = RowValue(win, Y); RowLabel(win, "Aspect Ratio",              Y); Y = Y - ROW_H
    win._vScale   = RowValue(win, Y); RowLabel(win, "UIParent Scale",            Y); Y = Y - ROW_H
    win._vTarget  = RowValue(win, Y); RowLabel(win, "Pixel-Perfect Target",      Y); Y = Y - ROW_H
    win._vPixelU  = RowValue(win, Y); RowLabel(win, "1 Pixel  (region units)",   Y); Y = Y - ROW_H

    Y = Y - 4;  HLine(win, Y);  Y = Y - GAP

    -- =========================================================================
    -- SECTION: SCALE CONTROLS
    -- =========================================================================
    SectionTitle(win, "Scale Controls", Y);  Y = Y - 20

    local cbEnable = MakeCheckbox(win, "Enable Pixel Perfect Mode", PAD + 2, Y)
    ...
    local cbAuto = MakeCheckbox(win, "Auto-apply on login and display changes", PAD + 2, Y)
    cbAuto:SetScript("OnClick", function(self) PPUI.db.autoApply = self:GetChecked() end)
    win._cbAuto = cbAuto;  Y = Y - 24

    local cbManual = MakeCheckbox(win, "Override with manual scale value", PAD + 2, Y)
    ...
    -- Slider row: "Manual Scale:" label   [=====|====] value
    RowLabel(win, "Manual Scale:", Y);
    win._sliderValFS = RowValue(win, Y);  Y = Y - ROW_H

    local slider = MakeSlider(win)
    slider:SetPoint("TOPLEFT",  win, "TOPLEFT",  PAD + 2, Y)
    slider:SetPoint("TOPRIGHT", win, "TOPRIGHT", -(PAD + 2), Y)
    ...
    -- Tip under slider showing what the optimal value would be
    win._optimalHintFS = FS(win, 10)
    win._optimalHintFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    win._optimalHintFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])
    Y = Y - 16

    -- Button row
    local btnApply = MakeButton(win, "Apply Now", 108, 22)
    btnApply:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 2, Y)
    btnApply:SetScript("OnClick", function()
        PPUI:ApplyScale()
        PPUI:Log("Scale applied.")
        win:Refresh()
    end)

    local btnOptimal = MakeButton(win, "Set Optimal", 108, 22)
    ...
    local btnReset = MakeButton(win, "Reset Default", 108, 22)
    ...
    Y = Y - 32;  HLine(win, Y);  Y = Y - GAP

    -- =========================================================================
    -- SECTION: ELVUI INTEGRATION
    -- =========================================================================
    SectionTitle(win, "ElvUI Integration", Y);  Y = Y - 20

    win._elvStatusFS = FS(win, 11)
    win._elvStatusFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    Y = Y - ROW_H

    win._vElvScale  = RowValue(win, Y);  RowLabel(win, "E.uiScale",         Y);  Y = Y - ROW_H
    win._vElvMult   = RowValue(win, Y);  RowLabel(win, "E.mult",            Y);  Y = Y - ROW_H
    win._vElvIdeal  = RowValue(win, Y);  RowLabel(win, "E.mult  (ideal)",   Y);  Y = Y - ROW_H
    win._vElvMatch  = RowValue(win, Y);  RowLabel(win, "Sync status",       Y);  Y = Y - ROW_H + 2

    -- ElvUI note explaining E.mult
    local elvNoteFS = FS(win, 10)
    elvNoteFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    elvNoteFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])
    elvNoteFS:SetText("E.mult drives 1px borders/padding — wrong value = blurry edges and gaps")
    Y = Y - 16

    local cbHook = MakeCheckbox(win, "Hook UpdateUIScale (prevent ElvUI from overriding scale)", PAD + 2, Y)
    cbHook:SetScript("OnClick", function(self)
        PPUI.db.hookElvUI = self:GetChecked()
        if PPUI.db.hookElvUI then PPUI:HookElvUI() end
    end)
    win._cbHookElvUI = cbHook;  Y = Y - 26

    local btnSync = MakeButton(win, "Force Sync ElvUI", 148, 22)
    btnSync:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 2, Y)
    ...
    -- Reload UI button (sometimes needed after ElvUI sync to repaint borders)
    local btnReload = MakeButton(win, "ReloadUI", 90, 22)
    ...
    -- SECTION: DIAGNOSTICS
    -- =========================================================================
    SectionTitle(win, "Diagnostics", Y);  Y = Y - 20

    win._vCvarScale = RowValue(win, Y);  RowLabel(win, "uiScale CVar",          Y);  Y = Y - ROW_H
    win._vCvarUse   = RowValue(win, Y);  RowLabel(win, "useUiScale CVar",       Y);  Y = Y - ROW_H
    win._vEffScale  = RowValue(win, Y);  RowLabel(win, "Effective Scale",       Y);  Y = Y - ROW_H
    win._vDensity   = RowValue(win, Y);  RowLabel(win, "Pixel Density (N)",     Y);  Y = Y - ROW_H
    win._vPxPerU    = RowValue(win, Y);  RowLabel(win, "Pixels per unit",       Y);  Y = Y - ROW_H
    win._vDrift     = RowValue(win, Y);  RowLabel(win, "Sub-pixel drift",       Y);  Y = Y - ROW_H
    win._vBypass    = RowValue(win, Y);  RowLabel(win, "CVar bypass needed",    Y);  Y = Y - ROW_H

    Y = Y - 6

    -- ── Pixel snap calculator ─────────────────────────────────────────────
    local snapLabelFS = FS(win, 11)
    snapLabelFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    snapLabelFS:SetText("Snap test:")
    snapLabelFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])

    local snapEB = CreateFrame("EditBox", nil, win, "InputBoxTemplate")
    ...
    local snapResFS = FS(win, 11)
    snapResFS:SetPoint("LEFT", snapEB, "RIGHT", 8, 1)
    win._snapResFS = snapResFS

    local snapBtn = MakeButton(win, "Snap", 52, 22)
    snapBtn:SetPoint("RIGHT", win, "RIGHT", -(PAD + 2), Y)
    ...
    -- ── Useful hint about snap usage ─────────────────────────────────────
    local snapHintFS = FS(win, 10)
    snapHintFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    snapHintFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])
    snapHintFS:SetText("Enter any region-unit value to see its snapped equivalent and pixel count")
    Y = Y - 16

    -- ── Alignment Tools section ────────────────────────────────────────────
    Y = Y - 32;  HLine(win, Y);  Y = Y - GAP

    SectionTitle(win, "Alignment Tools", Y);  Y = Y - 20

    -- Grid controls
    local cbGrid = MakeCheckbox(win, "Pixel grid overlay", Y)
    cbGrid:SetChecked(PPUI.db and PPUI.db.gridEnabled or false)
    ...
    local gridSizeFS = FS(win, 11)
    gridSizeFS:SetPoint("LEFT", win, "LEFT", PAD + 190, Y + 1)
    gridSizeFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])
    gridSizeFS:SetText("Grid px:")

    -- Grid size buttons: 8 / 16 / 32 / 64
    local gridX = PAD + 250
    for _, sz in ipairs({ 8, 16, 32, 64 }) do
        local gb = MakeButton(win, tostring(sz), 32, 18)
        gb:SetPoint("TOPLEFT", win, "TOPLEFT", gridX, Y)
        local szCapture = sz
    ...
    -- Guide controls
    local btnHGuide = MakeButton(win, "Add H Guide", 100, 22)
    btnHGuide:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 2, Y)
    btnHGuide:SetScript("OnClick", function() PPUI:AddHGuide() end)

    local btnVGuide = MakeButton(win, "Add V Guide", 100, 22)
    btnVGuide:SetPoint("LEFT", btnHGuide, "RIGHT", 6, 0)
    btnVGuide:SetScript("OnClick", function() PPUI:AddVGuide() end)

    local btnClearGuides = MakeButton(win, "Clear Guides", 100, 22)
    ...
    -- Centre indicator
    local cbCenter = MakeCheckbox(win, "Screen centre crosshair", Y)
    cbCenter:SetChecked(PPUI.db and PPUI.db.showCenter or false)
    cbCenter:SetScript("OnClick", function(self)
        PPUI:ToggleCenterIndicator()
        self:SetChecked(PPUI.db.showCenter)
    end)
    win._cbCenter = cbCenter

    Y = Y - 22

    local alignHintFS = FS(win, 10)
    alignHintFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 4, Y)
    alignHintFS:SetTextColor(T.dim[1], T.dim[2], T.dim[3])
    alignHintFS:SetText("Guides: drag to reposition · right-click to remove · /pp hguide /pp vguide")
    Y = Y - 14

    -- ── Bottom action bar ─────────────────────────────────────────────────
    Y = Y - 8
    local btnInfo = MakeButton(win, "Print Info to Chat", 148, 22)
    btnInfo:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 2, Y)
    btnInfo:SetScript("OnClick", function() PPUI:PrintFullInfo() end)

    local btnInspect = MakeButton(win, "Frame Inspector", 130, 22)
    btnInspect:SetPoint("LEFT", btnInfo, "RIGHT", 8, 0)
    btnInspect:SetScript("OnClick", function() PPUI:ToggleInspector() end)

    local helpFS = FS(win, 10)
    helpFS:SetPoint("TOPLEFT", win, "TOPLEFT", PAD + 2, Y - 22)
    helpFS:SetText("|cff555555/pp help  for all slash commands  •  /pp inspect  to pick a frame|r")

    Y = Y - 32

    -- Set final window height to fit content
    win:SetHeight(math.abs(Y) + 6)
    ...
    function win:Refresh()
        ...
        local frac    = pxPerU - math.floor(pxPerU + 0.00001)  -- float-safe
        ...
        elseif perfect then
            ...
            local density = N == 1 and "1px/unit" or string.format("%dpx/unit  (×%d density)", N, N)
            self._statusFS:SetText(string.format("|cff33dd55● PIXEL PERFECT  |r|cff888888%s|r", density))
        else
        ...
        self._optimalHintFS:SetText(string.format(
            "Optimal: |c%s%.6f|r  (N=%d, %d px/unit, same visual size as ~%dpx screen)",
    ...
    -- Gentle 2.5s periodic refresh: catches external scale changes (e.g. ElvUI fighting back)
...
-- Override the stub in Core.lua now that GUI.lua has been loaded
